...
class MotorNode(Node):
    ...
    def JointAngle2MotorPos(self, JointAngles, M0, lambda_=0.01, max_iter=200, tol=1e-3):
        ...
        #M = np.array([Motor1, Motor5, SolvedM1toM4[2], SolvedM1toM4[3], SolvedM1toM4[1], Motor6])
    ...
    def jacobian_motor(self,M, h=1e-6):
        ...
        for i in range(n):
            Mp = M.copy()
            Mp[i] += h
            #qp = self.MotorPos2JointAngle(Mp)
    ...
    def Motor1to4_dls(self, Joint3and5and6, M0, lam=0.1, max_iter=200, tol=1e-5):
        ...
        """
        Numerical Jacobian of q = Motor2JointAngle(M)
        Returns a matrix J (m x n)
        """
        M = M.astype(float)
        q0 = self.MotorPos2JointAngle(M)
    ...
    def ForwardK_with_Arrays(self, Alpha_Array, a_array, d_array, theta_array):
        ...
        return Whole_FK_matrix    
    def IK(self, T_des, q0,
        max_iter=200, lambda_=0.1,
        tol_pos=1e-4, tol_ori=1e-4):
        """
        ...
        for _ in range(max_iter):
            ...
            J = self.jacobian_numeric(q) # shape (6,N)             # ---- Compute numeric Jacobian ----
    ...
    def rotm2axang(self, Rm):
        ...
        return axis, angle
        ################
        angle = np.arccos((np.trace(Rm) - 1) / 2)
        if angle < 1e-6:
            # No rotation
            return np.array([1, 0, 0]), 0.0
        elif np.pi - angle < 1e-6:
            # 180 degree rotation
            rx = np.sqrt((Rm[0, 0] + 1) / 2)
            ry = np.sqrt((Rm[1, 1] + 1) / 2)
            rz = np.sqrt((Rm[2, 2] + 1) / 2)
