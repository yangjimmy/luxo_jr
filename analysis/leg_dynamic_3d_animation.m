...
t1_l = linspace(-5, 5, steps);
t2_l = linspace(-90+5, -90-5, steps);
t4_l = linspace(-5, 5, steps);
t1_d_l = linspace(5, -5, steps);
t2_d_l = linspace(-90-5, -90+5, steps);
t7_l = linspace(90, -90, steps);

vstream = VideoWriter("animation.mp4","MPEG-4");
...
for i = 1:steps
    ...
    t1 = deg2rad(t1_l(i));
    t2 = deg2rad(t2_l(i));
    t1_d = deg2rad(t1_d_l(i));
    t2_d = deg2rad(t2_d_l(i));
    t4 = deg2rad(t4_l(i));
    t7 = deg2rad(t7_l(i));
